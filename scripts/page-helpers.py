"""Shared page-generation helpers for Studio One Fresno.

Usage in a Python script:
    from page_helpers import HEAD, HEADER_NAV, FOOTER_PARTIAL, FOOTER_SCRIPT

These constants are f-string-ready (use double braces for literal braces).
"""

# Shared <head> without the closing tag. Append page-specific <style> and close </head>.
HEAD = """<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <!-- Consent Mode + GTM + GA4 + Facebook Pixel -->
  <script>
  ...
  <!-- Third-party tags deferred until first interaction (performance) -->
  <script>
  (function(){{var l=0;function L(){{if(l)return;l=1;
  (function(w,d,s,a,i){{w[a]=w[a]||[];w[a].push({{'gtm.start':new Date().getTime(),event:'gtm.js'}});var f=d.getElementsByTagName(s)[0],j=d.createElement(s);j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i;f.parentNode.insertBefore(j,f);}})(window,document,'script','dataLayer','GTM-WQGCMZ9Q');
  ...
  <style>
    *,*::before,*::after{{margin:0;padding:0;box-sizing:border-box}}
    ...
    .primary-button{{display:inline-block;padding:16px 32px;border:1px solid var(--olive);color:var(--light-gray);font-size:12px;font-weight:500;letter-spacing:3px;text-transform:uppercase;font-family:var(--body-font);transition:background .3s,color .3s}}
    ...
    .section-label{{font-size:11px;font-weight:600;letter-spacing:3px;text-transform:uppercase;color:var(--olive-light);margin-bottom:16px}}
...
FOOTER_CTA = """
  <!-- Closing CTA -->
  <section class="section" style="background:var(--black);text-align:center">
    <div class="section-inner" data-animate>
      <h2 class="section-heading" style="max-width:720px;margin:0 auto 20px">{cta_heading}</h2>
      <p style="font-size:16px;color:var(--light-gray);max-width:560px;margin:0 auto 32px;font-weight:300">{cta_body}</p>
      <a href="{cta_url}" class="olive-button"{cta_target}>{cta_text}</a>
    </div>
  </section>
"""
